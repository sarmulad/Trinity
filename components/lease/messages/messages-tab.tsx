      ...
      <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-md p-5 space-y-3">
        <div className="relative w-72">
          ...
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-[#252930] py-2 pl-9 pr-3 text-sm text-white placeholder:text-white/30 focus:border-[#34C759]/50 focus:outline-none"
          />
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2">
            {filtered.map((msg) => (
              <div
                key={msg.id}
                className="rounded-xl border border-white/10 bg-[#1e2127] p-4"
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2.5">
                    <Avatar className="h-8 w-8 shrink-0">
                      <AvatarImage src={msg.authorAvatar} />
                      <AvatarFallback className="bg-[#2d3440] text-xs text-white">
                        {msg.authorInitials}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                  ...
                  <p className="text-xs text-white/30 shrink-0">{msg.date}</p>
                </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <textarea
            ...
            rows={4}
            className="w-full rounded-lg border border-white/10 bg-[#252930] px-4 py-3 text-sm text-white placeholder:text-white/25 focus:border-[#34C759]/50 focus:outline-none resize-none"
          />
