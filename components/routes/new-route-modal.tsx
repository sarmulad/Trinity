...
const STOP_SUGGESTIONS: RouteStop[] = [
  { id: "1", name: "Riordan Lease", distance: "0.2 miles", coordinates: "35.6870° N, 105.9378° W" },
  { id: "2", name: "Teasley Lease", distance: "1.2 miles", coordinates: "35.6870° N, 105.9378° W" },
  { id: "3", name: "Union Lease", distance: "0.2 miles", coordinates: "35.6870° N, 105.9378° W" },
  { id: "4", name: "Riffle Lease", distance: "0.3 miles", coordinates: "35.6870° N, 107.9378° W" },
  { id: "5", name: "River Estate Lease", distance: "0.2 miles", coordinates: "35.6870° N, 105.9378° W" },
];
...
export function NewRouteModal({
  ...
  const addStop = (stop: RouteStop) => {
    setStops((prev) => (prev.some((s) => s.id === stop.id) ? prev : [...prev, stop]));
    setStopSearch("");
    setSuggestionsOpen(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-lg border-white/10 bg-[#1A1C1E] p-0 text-white [&>button]:hidden"
      >
        ...
        <div className="space-y-5 px-6 pb-6">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder={stops.length === 0 ? "Select stop 1" : "Search stops..."}
                className="border-white/20 bg-[#252930] pl-9 text-white placeholder:text-white/40"
              />
              {suggestionsOpen && (filteredSuggestions.length > 0 || stopSearch) && (
                <>
                  <div
                  ...
                  />
                  <div className="absolute top-full left-0 right-0 z-10 mt-1 max-h-48 overflow-auto rounded-md border border-white/10 bg-[#252930] py-1">
                    {filteredSuggestions.map((stop) => (
                      <button
                        key={stop.id}
                        type="button"
                        onClick={() => addStop(stop)}
                        className="flex w-full flex-col gap-0.5 px-3 py-2.5 text-left text-sm text-white hover:bg-white/10"
                      >
                        <span>
                          {stop.name} ({stop.distance})
                        </span>
                        <span className="text-xs text-white/50">{stop.coordinates}</span>
                      </button>
                    ))}
            ...
            {stops.length > 0 && (
              <div className="space-y-2 pt-1">
                ...
                <p className="flex items-center gap-1.5 text-sm text-white/60">
                  <GripVertical className="h-3.5 w-3.5 text-[#34C759]" aria-hidden />
                  <span>Click and drag a stop to reorder route</span>
                </p>
                <ul className="space-y-2">
                  {stops.map((stop, index) => (
                    <li
                      ...
                      draggable
                      onDragStart={(e) => {
                        e.dataTransfer.setData("application/json", JSON.stringify({ id: stop.id }));
                      ...
                      onDrop={(e) => {
                        ...
                        const fromIndex = stops.findIndex((s) => s.id === draggedId);
                      ...
                      className="group flex cursor-grab items-center justify-between gap-2 rounded-md border border-white/10 bg-[#252930] px-3 py-2 active:cursor-grabbing"
                    >
            ...
            >
              <span className="flex h-5 w-5 items-center justify-center rounded-full border border-[#34C759] text-[#34C759]">
          ...
          <div className="flex w-full flex-col gap-3 border-t border-white/10 pt-4">
            ...
            <Button
              type="button"
              className="w-full border border-[#FF383C] bg-transparent text-center font-semibold text-[#FF383C] hover:bg-[#FF383C]/10"
